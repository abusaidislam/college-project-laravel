...
class StudentResultThirdYearController extends Controller
{
    public function index()
    {
        ...
        $Course   = CourseName::where('class_id', 3)->orderby('name','asc')->get();
       
        $studentclass = StudenClass::where('id', '=', 3)->first();
        $Student      = StudentHonoursThirdYear::where('depart_id', '=', $depart_id)->where('class_id',3)->latest()->get();
        $StudentResult = StudentResult::where('depart_id', '=', $depart_id)->where('class_id',3)->latest()->get();

        
        $Studentlist      = Student::where('depart_id', '=', $depart_id)->get();
        Session::put('depart_id', $depart_id);
        Session::put('depart_name', $depart_name);
        return view('backend.studentresultthirdyear', compact('depart_id','Course','Studentlist','depart_name','studentinfo','Student','CourseName','classname','studentclass'));
    }
    ...
    public function store(Request $request){ 
      ...
      Session::put('depart_name', $depart_name);
       ...
       return redirect('students-result-third-year')->with('massage', 'Inserted successfully!!!');
 ...
 return $studentclass;
    ...
    public function StudentThirdYear($id){  
      $student_year = DB::table('student_honours_third_years')->where('class_id', $id)->orderBy('session_year','desc')->get(); 
      return response()->json($student_year);
    
 }
    public function getUserbyidthird(Request $request){
      $depart_id    = Session::get('depart_id');
    ...
    {
        StudeStudentHonoursThirdYearnt::find($id)->delete();
 ...
 public function allresult()
    ...
    public function thirdYearMarksheet($registration_no)
    {
      $studentInfos = Student::where('registration_no', $registration_no)->first();
      $studentInfo = StudentHonoursThirdYear::where('registration_no',$registration_no)->first();
        $stuinfo = DB::table('student_honours_third_years')  
      ...
      $examyear= DB::table('student_results')  
          ...
          ->first();
      $Course = DB::table('student_honours_third_years')
                ->where('student_honours_third_years.id', $studentInfo->id)
                ->where('student_honours_third_years.class_id', $studentInfo->class_id)
       ...
       return view('backend.mark_sheet_3rdyear_download', compact('Course','studentInfos','examyear'));
    }
