...
class DegreeInCourseMarksThirdYearController extends Controller
{
    public function index()
    {
        ...
        return view('backend.degree_incourse_mark_thirdyear', compact('StudentIncourse','studentclass','studentsession','Course'));
      }
    ...
    for ($i = 0; $i < count($written_marks); $i++) {
        ...
        if ($dataInserts) {
            if ($request->id != 0) {
                return redirect('degree-third-year-incourse-mark')->with('message', 'Updated successfully!!!');
            } else {
                return redirect('degree-third-year-incourse-mark')->with('message', 'Inserted successfully!!!');
            }
        } else {
            return redirect('degree-third-year-incourse-mark')->with('message', 'Error!!!');
        }
    ...
    public function degreeIncourseThird(Request $request){
     ...
     public function degreeThirdYearExportToPDF(Request $request)
     {
       ...
       $data = DegreeIncourseMarks::where('class_id', $class)
         ...
         $pdf = PDF::loadView('backend.degreeincourse_third_year_pdf', ['data' => $data,'subject' => $subject,'class' => $class]);
