...
class HostelLogoSignController extends Controller
{
    public function index()
    {
       $data = LibraryLogo::where('id',4)->orderBy('id', 'desc')->get();
        return view('backend.hostel_idcard_logo_sing', compact('data'));
    }
    ...
    $request->validate([
        ...
        try {
            ...
            if ($request->id != 0) {
                return redirect('hostel_logosign')->with('message', 'Updated successfully!!!');
            } else {
                return redirect('hostel_logosign')->with('message', 'Inserted successfully!!!');
            }
        } catch (\Exception $e) {
            return redirect('hostel_logosign')->with('error', 'The form was not filled up completely!!!');
