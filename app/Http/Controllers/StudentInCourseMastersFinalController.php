...
use PDF;
class StudentInCourseMastersFinalController extends Controller
{
    public function index()
    {
        ...
        $studentsession = StudentMastersFinal::select('session')->where('studentclass', $studentclass->id)->distinct()->limit(10)->get();
        ...
        return view('backend.studentincoursemasterfinal', compact('depart_id','studentsession','StudentIncourse','depart_name','studentclass','Course'));
      }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < count($written_marks); $i++) {
            ...
            if ($dataInserts) {
                if ($request->id != 0) {
                    return redirect('masters-final-incourse-mark')->with('message', 'Updated successfully!!!');
                } else {
                    return redirect('masters-final-incourse-mark')->with('message', 'Inserted successfully!!!');
                }
            } else {
                return redirect('masters-final-incourse-mark')->with('message', 'Error!!!');
            }
    }
    ...
    public function getIncourseMaster(Request $request){
     ...
     public function MasterFinalYearExportToPDF(Request $request)
     {
        ...
        $pdf = PDF::loadView('backend.studentincourse_masterfinal_year_pdf', ['data' => $data,'depart_id' => $depart_id,'subject' => $subject,'class' => $class]);
