...
use PDF;
class StudentInCoursePreliminaryController extends Controller
{
    public function index()
    {
        ...
        $studentsession = StudentPreliminaryToMasters::select('session')->where('studentclass', $studentclass->id)->distinct()->limit(10)->get();
        ...
        return view('backend.studentincoursepreliminary', compact('depart_id','studentsession','StudentIncourse','depart_name','studentclass','Course'));
      }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < count($written_marks); $i++) {
            ...
            if ($dataInserts) {
                if ($request->id != 0) {
                    return redirect('preliminary-incourse-mark')->with('message', 'Updated successfully!!!');
                } else {
                    return redirect('preliminary-incourse-mark')->with('message', 'Inserted successfully!!!');
                }
            } else {
                return redirect('preliminary-incourse-mark')->with('message', 'Error!!!');
            }
    }
    ...
    public function getIncoursePreliminary(Request $request){
     ...
     public function PreliminaryYearExportToPDF(Request $request)
     {
        ...
        $pdf = PDF::loadView('backend.studentincourse_preliminary_year_pdf', ['data' => $data,'depart_id' => $depart_id,'subject' => $subject,'class' => $class]);
