...
class SeminarLogoSignController extends Controller
{
    public function index()
    {
       $data = LibraryLogo::where('id',6)->orderBy('id', 'desc')->get();
        return view('backend.seminar_logo&sign', compact('data'));
    }
    ...
    $request->validate([
        ...
        try {
            ...
            if ($request->id != 0) {
                return redirect('seminar-logo-sign')->with('message', 'Updated successfully!!!');
            } else {
                return redirect('seminar-logo-sign')->with('message', 'Inserted successfully!!!');
            }
        } catch (\Exception $e) {
            return redirect('seminar-logo-sign')->with('error', 'The form was not filled up completely!!!');
