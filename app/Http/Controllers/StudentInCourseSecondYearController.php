...
use PDF;
class StudentInCourseSecondYearController extends Controller
{
    public function index()
    {
        ...
        $studentsession = StudentHonoursSecoundYear::select('session_year')->where('class_id', $studentclass->id)->distinct()->limit(10)->get();
        ...
        return view('backend.studentincoursesecondyear', compact('depart_id','studentsession','StudentIncourse','depart_name','studentclass','Course'));
    }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < count($written_marks); $i++) {
            ...
            if ($dataInserts) {
                if ($request->id != 0) {
                    return redirect('second-year-incourse-mark')->with('message', 'Updated successfully!!!');
                } else {
                    return redirect('second-year-incourse-mark')->with('message', 'Inserted successfully!!!');
                }
            } else {
                return redirect('second-year-incourse-mark')->with('message', 'Error!!!');
            }
    }
    ...
    public function getIncourseSecound(Request $request){
    ...
    foreach ($Students as $value) {
     ...
     public function SecondYearExportToPDF(Request $request)
     {
        ...
        $data = StudentIncourseMarks::where('class_id', $class)
         ...
         $pdf = PDF::loadView('backend.studentincourse_second_year_pdf', ['data' => $data,'depart_id' => $depart_id,'subject' => $subject,'class' => $class]);
