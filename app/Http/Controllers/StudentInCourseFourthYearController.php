...
use PDF;
class StudentInCourseFourthYearController extends Controller
{
    public function index()
    {
        ...
        $studentsession = StudentHonoursFourthYear::select('session_year')->where('class_id', 4)->distinct()->limit(10)->get();
        ...
        return view('backend.studentincoursefourthyear', compact('depart_id','studentsession','StudentIncourse','depart_name','studentclass','Course'));
      }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 0; $i < count($written_marks); $i++) {
            ...
            if ($dataInserts) {
                if ($request->id != 0) {
                    return redirect('fourth-year-incourse-mark')->with('message', 'Updated successfully!!!');
                } else {
                    return redirect('fourth-year-incourse-mark')->with('message', 'Inserted successfully!!!');
                }
            } else {
                return redirect('fourth-year-incourse-mark')->with('message', 'Error!!!');
            }
    }
    ...
    public function getIncourseFourth(Request $request){
     ...
     public function FourthYearExportToPDF(Request $request)
     {
        ...
        $pdf = PDF::loadView('backend.studentincourse_fourth_year_pdf', ['data' => $data,'depart_id' => $depart_id,'subject' => $subject,'class' => $class]);
