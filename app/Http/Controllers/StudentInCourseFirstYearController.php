...
use PDF;
class StudentInCourseFirstYearController extends Controller
{
    public function index()
    {
        ...
        $studentsession = Student::select('session')->where('studentclass', $studentclass->id)->orderBy('session','desc')->distinct()->limit(10)->get();
        ...
        return view('backend.studentincoursefirstyear', compact('depart_id','studentsession','StudentIncourse','depart_name','studentclass','Course'));
      }
    ...
    for ($i = 0; $i < count($written_marks); $i++) {
        ...
        if ($dataInserts) {
            if ($request->id != 0) {
                return redirect('first-year-incourse-mark')->with('message', 'Updated successfully!!!');
            } else {
                return redirect('first-year-incourse-mark')->with('message', 'Inserted successfully!!!');
            }
        } else {
            return redirect('first-year-incourse-mark')->with('message', 'Error!!!');
        }
    ...
    public function getIncourseFirst(Request $request){
        ...
        foreach ($Students as $value) {
          ...
          <td align="center">
           ...
           <td align="center">
           <input type="text" class="form-control" id="marks" name="marks[]" placeholder="Enter (0-15) Marks" value="" required="" min="0" max="15">
           </td>
           <td align="center">
     ...
     public function FirstYearExportToPDF(Request $request)
     {
         ...
         $pdf = PDF::loadView('backend.studentincourse_first_year_pdf', ['data' => $data, 'depart_id' => $depart_id,'subject' => $subject,'class' => $class]);
