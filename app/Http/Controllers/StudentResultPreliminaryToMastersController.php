...
class StudentResultPreliminaryToMastersController extends Controller
{
    public function index()
    {
        ...
        $Course   = CourseName::where('class_id', 5)->orderby('name','asc')->get();
       
        $studentclass = StudenClass::where('id', '=', 5)->first();
        $Student      = StudentPreliminaryToMasters::where('depart_id', '=', $depart_id)->where('studentclass',5)->latest()->get();
        $StudentResult = StudentResult::where('depart_id', '=', $depart_id)->where('class_id',5)->latest()->get();
        
        $Studentlist      = Student::where('depart_id', '=', $depart_id)->get();
        Session::put('depart_id', $depart_id);
        Session::put('depart_name', $depart_name);
        return view('backend.studentresult_preliminarytomaster', compact('depart_id','Course','StudentResult','Studentlist','depart_name','studentinfo','Student','CourseName','classname','studentclass'));
      }
    ...
    public function store(Request $request) { 
      ...
      return redirect('preliminary-masters-result')->with('message', 'Inserted successfully!!!');
  }
 ...
 public function searchsubjet($id)
    ...
    public function edit($id)
    {
       ...
       public function studentPreliminary($id){  
  
        $student_year = DB::table('student_preliminary_to_masters')->where('studentclass', $id)->orderBy('session','desc')->get(); 
        return response()->json($student_year);
   }

    public function getUserbyidPreliminary(Request $request){
      $depart_id    = Session::get('depart_id');
      $userid         = $request->userid;
      $data = explode('/', $userid);  
      $class_id = $data[0]; 
      $class_year = $data[1]; 
      $Students = DB::table('student_preliminary_to_masters')
 ...
 public function allresult()
    ...
    public function preliminaryMarksheet($registration_no)
    {

     $studentInfo = StudentPreliminaryToMasters::where('registration_no',$registration_no)->first();
     $studentInfos = StudentPreliminaryToMasters::where('registration_no', $registration_no)->first();
     $stuinfo = DB::table('student_preliminary_to_masters')  
     ...
     ->first();                                 
    $CourseIn = DB::table('student_preliminary_to_masters')
                        ->where('student_preliminary_to_masters.id', $studentInfo->id)
                        ->where('student_preliminary_to_masters.studentclass', $studentInfo->studentclass)
                        ->where('course_names.depart_id', $studentInfo->depart_id)
                        ->join('course_names', 'student_preliminary_to_masters.studentclass', '=', 'course_names.class_id')
                        ->select('student_preliminary_to_masters.*', 'course_names.id AS course_id','course_names.name AS course_name','course_names.course_code','course_names.credit','course_names.marks' )
                        ->get();
 
      return view('backend.mark_sheet_preliminary_download', compact('CourseIn','studentInfos','examyear'));
    }
