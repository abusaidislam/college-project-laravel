...
@section('title', 'Degree Id Card Note Manage |')
@section('content')
<div class="content">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('degree_idcard_note.store') }}" enctype="multipart/form-data">
                                        @csrf()

                                        <input type="hidden" name="id" id="id">
                                        <div class="form-group">
                                            <label for="idcard_note" class="col-sm-12 control-label">ID Card
                                                Note</label>
                                            <div class="col-sm-12">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('degree_idcard_note.index') }}" + '/' + id + '/edit', function(data) {
                $('#modelHeading').html("Edit");
                $('#saveBtn').html('Update');
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#idcard_note').val(data.note);
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'degree_idcard_note/' + id,
                        success: function(data) {
                            window.location = 'degree_idcard_note';
