@extends('layouts.app')
@section('title', 'Other Publications Manage |')
@section('content')
<div class="content">
                                        ...
                                        <tbody id="list">
                                            @foreach ($data as $ndata)
                                                <tr>
                                                    ...
                                                    <td>{{ $ndata->title }}</td>
                                                    <td>
                                                        <a href="{{ asset('public/other-publication/' . $ndata->file) }}" download
                                                            class="btn btn-info ">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('other-publication.store') }}" enctype="multipart/form-data">
                                        @csrf()
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('other-publication.index') }}" + '/' + id + '/edit', function(data) {
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'other-publication/' + id,
                        success: function(data) {
                            window.location = 'other-publication';
