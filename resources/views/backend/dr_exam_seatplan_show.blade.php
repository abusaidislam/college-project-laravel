            ...
            <div class="x_panel" id="seaplan">
                ...
                <div class="row" width="100%">
                    @foreach ($College_Seatdetails as $item)
                        @php
                            $SeatInfo = DB::table('exam_dr_seat_seatings')
                                ->where('collegee_name', $item->collegee_name)
                                ->where('exam_year', $item->exam_year)
                                ->where('exam_routin_id', $item->exam_routin_id)
                                ->first();
                            $rollinfo = DB::table('exam_dr_seat_seatings')
                                ->select('roll')
                                ->where('collegee_name', $item->collegee_name)
                                ->where('exam_year', $item->exam_year)
                                ->where('exam_routin_id', $item->exam_routin_id)
                        ...
                        <h6 style="margin-left: 50px; color:rgb(205, 0, 174);">Total = {{ $rollinfo ? $rollinfo : '' }},
                            ...
                            ,
                            College Name: {{ $SeatInfo ? $SeatInfo->collegee_name : '' }}
