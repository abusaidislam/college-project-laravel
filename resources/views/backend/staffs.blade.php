...
<div class="content">
                                        ...
                                        <tbody id="list">
                                            @foreach ($data as $ndata)
                                                <tr>
                                                    ...
                                                    <td><img src="{{ asset('public/Staff/' . $ndata->photo) }} "
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal modal-lg"
                                        action="{{ route('department_staff.store') }}" enctype="multipart/form-data">
                                        @csrf()
                                        <input type="hidden" name="depart_id" id="depart_id"
                                            value="{{ Session::get('depart_id') }}">
                                        <input type="hidden" name="id" id="id">
                                        @php
                                            $number = DB::table('staff')
                                                ->select('serial_num')
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-12">
                                                        <input type="number" class="form-control" id="serial_num" name="serial_num"
                                                            value="{{ $number->serial_num + 1 }}" required="">
                                                    </div>
                                                </div>
                                                <div class="form-group">
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            ...
            $.get("{{ route('department_staff.index') }}" + '/' + id + '/edit', function(data) {
                $('#ajaxModel').modal('show');
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'department_staff/' + id,
                        success: function(data) {
                            window.location = 'department_staff';
