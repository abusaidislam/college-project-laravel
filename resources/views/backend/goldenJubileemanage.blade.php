@extends('layouts.app')
@section('title', 'Golden Jubilee Manage |')
@section('content')
    <div class="content">
                                            ...
                                            <tbody id="list">
                                                @foreach ($data as $ndata)
                                                    <tr>
                                                        ...
                                                        <td>{{ $ndata->title }}</td>
                                                        <td><img src="{{ asset('public/goldenJubilee/' . $ndata->image) }} "
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form method="POST" id="form" name="form" class="form-horizontal"
                                            action="{{ route('goldenJubileemanage.store') }}" enctype="multipart/form-data">
                                            @csrf()
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('goldenJubileemanage.index') }}" + '/' + id + '/edit', function(data) {
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'goldenJubileemanage/' + id,
                        success: function(data) {
                            window.location = 'goldenJubileemanage';
