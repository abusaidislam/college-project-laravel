        ...
        <style>
            ...
            * {
            ...
            .business2 .front {
                ...
                overflow: hidden;
                position: relative;
                background-image: url('{{ asset('public/library_card/bk1.jpg') }}');
            ...
            .business2 .back {
                ...
                overflow: hidden;
                position: relative;
                background-image: url('{{ asset('public/library_card/backbk1.jpg') }}');
                background-size: cover;
                background-position: center;
                            ...
                            @foreach ($ndata as $ndata)
                                <div class="business2" style="margin-top: -70px; margin-bottom:-90px">
                                    <div class="front">
                                        <div class="golsap">
                                            <div class="head">
                                                <img src="{{ asset('public/library_card/' . $basic->photo) }}"
                                                    alt="" srcset="" width="70" height="40">
                                                <div>
                                            ...
                                            @php
                                                $classInfo = DB::table('degree_classes')
                                                    ...
                                                    ->first();
                                                $stu_info = DB::table('degree_first_year_students')
                                                    ->where('registration_no', $ndata->registration_no)
                                                    ->where('session', $ndata->session_year)
                                                    ->first();
                                            @endphp
                                            <div class="avatars">
                                                <div class="img">

                                                    @if ($stu_info && $stu_info->photo)
                                                        <img src="{{ asset('public/student/' . $stu_info->photo) }}"
                                                            alt="" width="60px" height="80px">
                                                    @else
                                                        <img src="{{ asset('public/library_card/default.jfif') }}"
                                                            alt="" width="60px" height="80px">
                                                    @endif
                                                </div>
                                            ...
                                            <div class="avatersname">
                                                ...
                                                <p> <span style="border-bottom: 2px solid black">STUDENT ID CARD</span></p>
                                            </div>
                                            <div class="infos">
                                                <div>

                                                    <div>
                                                        <p>Card No : {{ $ndata->id }}</p>
                                                        <p>Roll : {{ $ndata->roll }}</p>
                                                        <p>Regi : {{ $ndata->registration_no }}</p>
                                                        <p>Dept : Department of Degree</p>
                                                        <p>Class : {{ $classInfo ? $classInfo->name : '' }}</p>
                                                        <p>Session : {{ $ndata->session_year }}</p>
                                                        <p>Mobile : {{ $stu_info ? $stu_info->mobile_no : '' }}</p>
                                                        <p>Blood Group : {{ $stu_info ? $stu_info->blood_group : '' }}</p>
                                    ...
                                    <div class="back">
                                        ...
                                        <?php
                                        $startDate = new DateTime($ndata->created_at);
                                        $endDate = $startDate->modify('+1 year');
                                        ?>
                                        <div class="date">
                                            <p> 
                                                {!! DNS1D::getBarcodeHTML(" $ndata->registration_no","PHARMA",1)!!}
                                                Create Date : {{ $ndata ? $ndata->created_at->format('Y-m-d') : '' }} <br>
