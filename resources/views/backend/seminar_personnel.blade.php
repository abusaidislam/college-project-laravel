...
@section('title', 'Seminar Personnel Manage |')
@section('content')
<div class="content">
                            ...
                            <h4 class="modal-title" id="modelHeading"></h4>
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('seminar-personnel.store') }}" enctype="multipart/form-data">
                                        @csrf()
                                        <input type="hidden" name="id" id="id">
                                        @php
                                            $number = DB::table('seminar_personnels')
                                                ->select('serial_num')
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('seminar-personnel.index') }}" + '/' + id + '/edit', function(data) {
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'seminar-personnel/' + id,
                        success: function(data) {
                            window.location = 'seminar-personnel';
