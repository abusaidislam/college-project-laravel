...
@section('title', 'Vice-Principal Details Manage |')
@section('content')
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>Vice-Principal Details Information</h2>
                                    ...
                                    <table id="datatable-responsive"
                                        ...
                                        <tbody id="list">
                                            @foreach ($data as $ndata)
                                                <?php $v_name = DB::table('viceprincipals')
                                                    ->where('id', $ndata->v_id)
                                                    ->get(); ?>
                                                @foreach ($v_name as $nv_name)
                                                    <tr>
                                                        <td id="v_name">{{ $nv_name->name }}</td>
                                                        <td>{{ $ndata->name }}</td>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal "
                                        action="{{ route('viceprincipaledetails.store') }}" enctype="multipart/form-data">
                                        @csrf()
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            ...
            $.get("{{ route('viceprincipaledetails.index') }}" + '/' + id + '/edit', function(data) {
                $('#ajaxModel').modal('show');
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'viceprincipaledetails/' + id,
                        success: function(data) {
                            window.location = 'viceprincipaledetails';
