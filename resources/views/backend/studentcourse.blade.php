...
<div class="content">
                                    ...
                                    <table id="datatable-responsive"
                                        ...
                                        <tbody id="list">
                                            @foreach ($data as $ndata)
                                                <tr>
                                                    <?php $classid = DB::table('studen_classes')
                                                        ->where('id', '=', $ndata->class_id)
                                                        ->get(); ?>
                                                    @foreach ($classid as $nclassid)
                                                        <td>{{ $nclassid->name }} </td>
                                                    @endforeach
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal modal-lg"
                                        action="{{ route('department-course.store') }}" enctype="multipart/form-data">
                                        @csrf()
                                        ...
                                        <div class="form-group">
                                            <label for="code" class="col-sm-12 control-label">Course Code</label>
                                            <div class="col-sm-12">
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            ...
            $.get("{{ route('department-course.index') }}" + '/' + id + '/edit', function(data) {
                $('#ajaxModel').modal('show');
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'department-course/' + id,
                        success: function(data) {
                            window.location = 'department-course';
