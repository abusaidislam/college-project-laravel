@extends('layouts.app')
@section('title', 'College Magazines Manage |')
@section('content')
<div class="content">
                                        ...
                                        <tbody id="list">
                                            @foreach ($data as $ndata)
                                                <tr>
                                                    ...
                                                    <td>{{ $ndata->title }}</td>
                                                    <td>
                                                        <a href="{{ asset('public/journal_magazin/' . $ndata->file) }}" download
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('college-magazine.store') }}" enctype="multipart/form-data">
                                        @csrf()
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('college-magazine.index') }}" + '/' + id + '/edit', function(data) {
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'college-magazine/' + id,
                        success: function(data) {
                            window.location = 'college-magazine';
