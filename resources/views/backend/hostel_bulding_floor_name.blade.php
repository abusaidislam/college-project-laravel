...
@section('title', 'Floor Name Manage |')
@section('content')
<div class="content">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('hostel-floor-name.store') }}" enctype="multipart/form-data">
                                        @csrf()
                                        ...
                                        <div class="form-group">
                                            <label for="name" class="col-sm-12 control-label">Floor Name </label>
                                            <div class="col-sm-12">
                                                <input type="text" class="form-control" id="floor_name" name="floor_name"
                                                    placeholder="Floor Name " value="" maxlength="50" required="">
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('hostel-floor-name.index') }}" + '/' + id + '/edit', function(data) {
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'hostel-floor-name/' + id,
                        success: function(data) {
                            window.location = 'hostel-floor-name';
