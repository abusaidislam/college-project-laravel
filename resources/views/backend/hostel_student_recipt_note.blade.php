...
@section('title', 'Student Receipt Note Manage |')
@section('content')
<div class="content">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form method="POST" id="form" name="form" class="form-horizontal"
                                        action="{{ route('student_recipt_note.store') }}" enctype="multipart/form-data">
                                        @csrf()

                                        <input type="hidden" name="id" id="id">
                                        <div class="form-group">
                                            <label for="name" class="col-sm-12 control-label">Recipt Note</label>
                                            <div class="col-sm-12">
                                                <input type="text" class="form-control" id="recipt_note"
                                                    name="recipt_note" placeholder="Enter Recipt Note" value=""
                                                    required="">
                                            </div>
                                        </div>
                                        <div class="form-group">
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '#edit', function() {
            var id = $(this).data('id');
            $.get("{{ route('student_recipt_note.index') }}" + '/' + id + '/edit', function(data) {
                $('#modelHeading').html("Edit");
                $('#saveBtn').html('Update');
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#recipt_note').val(data.recipt_note);
        ...
        $('body').on('click', '#delete', function() {
            ...
            Swal.fire(sweetAlertConfirmation).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: 'student_recipt_note/' + id,
                        success: function(data) {
                            window.location = 'student_recipt_note';
