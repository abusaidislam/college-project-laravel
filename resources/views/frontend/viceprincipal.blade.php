@extends('frontend.administrationlayout')
@section('administrationcontent')
    <div class="col-md-9  col-sm-9  border p-0 m-0 pb-5 ">
        <div class="row pt-3  col-md-12  col-sm-12 pb-4">
            <div class="col-sm-12 text-center">
                <h3 class="allfont"><span style="border-bottom: 3px solid rgb(41, 206, 227)">VICE PRINCIPAL</span>
                </h3>
            </div>
        </div>

        <div class="row ">
            <div class="col-sm-12">
                <div class="card-box">
                    <table id="" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr class="text-center">
                                <th class="text-center">Photo</th>
                                <th class="text-center">Name</th>
                                <th class="text-center">Designation</th>
                                <th class="text-center">Th</th>
                                <th class="text-center">Department</th>
                                <th class="text-center">BCS_Batch</th>
                                <th class="text-center">Mobile No.</th>
                                <th class="text-center">Blood Group</th>
                                <th class="text-center">Email</th>
                                <th class="text-center">Home District</th>

                            </tr>
                        </thead>
                        <tbody id="list">
                            @foreach ($viceprincipal as $ndata)
                                <tr class="text-center">
                                    <td class="text-center"><img src="{{ asset('public/viceprincipal/' . $ndata->photo) }} "
                                            alt="" width="100" height="80"> </td>
                                    <td>{{ $ndata->name }}</td>
                                    <td class="text-center">{{ $ndata->designation }} </td>
                                    <td class="text-center">{{ $ndata->th }} </td>
                                    <td class="text-center">{{ $ndata->department }} </td>
                                    <td class="text-center">{{ $ndata->bcs_batch }} Th </td>
                                    <td class="text-center">{{ $ndata->mobile_no }} </td>
                                    <td class="text-center">{{ $ndata->blood_group }}
                                    </td>
                                    <td class="text-center">{{ $ndata->email }} </td>
                                    <td class="text-center">{{ $ndata->home_dis }}
                                    </td>

                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    </div>
@endsection
